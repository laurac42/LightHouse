        ...
        <Card className="bg-highlight rounded-none lg:rounded-md shadow-md border-none lg:mb-6 fixed w-full bottom-0 z-50 left-0 lg:relative lg:w-auto lg:mb-0">
            <CardContent className="lg:p-4 p-2">
                ...
                <div className="flex flex-row items-center lg:contents">
                    ...
                    <div className="flex items-center flex-row gap-4 lg:pt-4">
                        <p className="justify-start hidden lg:block text-lg text-white/90 ml-2">{agencyDetails.address_line_1}, {agencyDetails.address_line_2 ? `${agencyDetails.address_line_2}, ` : ''}  {agencyDetails.city}, {agencyDetails.post_code}</p>
                        {agencyDetails.logo_url && (
                            <img src={agencyDetails.logo_url} alt={`${agencyDetails.name} Logo`} className="hidden sm:block max-w-32 md:max-w-48 mr-2 md:mr-6 h-12 md:h-16 lg:h-24 object-contain" />
                        )}
                    ...
                    <div className="flex flex-row gap-2 w-full sm:w-auto px-4 sm:px-2 lg:mt-8 sm:ml-auto sm:justify-end lg:justify-start">
                        <Button className="px-1 bg-yellow lg:mr-auto h-10 md:h-16 lg:h-20 w-1/2 sm:w-40 lg:w-1/2 hover:bg-yellowHover">
                            <Link href={`mailto:${agencyDetails.email}`} className="text-md md:text-lg text-foreground inline-flex lg:block xl:inline-flex items-center gap-1">
                                Email Agency
                                <Mail className="ml-2" />
                            </Link>
                        </Button>
                        <Button className="bg-yellow lg:ml-auto h-10 md:h-16 lg:h-20 w-1/2 sm:w-40 lg:w-1/2 hover:bg-yellowHover">
                            <Link href={`tel:${agencyDetails.phone_number}`} className="text-md md:text-lg text-foreground inline-flex lg:block xl:inline-flex items-center gap-1">
                                Call Agency
