...
CREATE INDEX index_properties_lat_long ON public.properties USING btree (latitude, longitude);

set check_function_bodies = off;

CREATE OR REPLACE FUNCTION public.fetch_ranked_properties(p_min_lat numeric, p_max_lat numeric, p_min_long numeric, p_max_long numeric, p_preferred_num_bedrooms smallint DEFAULT NULL::smallint, p_budget bigint DEFAULT NULL::bigint, p_preferred_property_types text[] DEFAULT NULL::text[], page integer DEFAULT 1, page_size integer DEFAULT 10)
...
begin
  ...
  with scored as (
    ...
    and p.longitude <= p_max_long
    and p.longitude >= p_min_long
  )
  select s.*
  from scored s
