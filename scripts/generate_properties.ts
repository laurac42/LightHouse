...
// a list of rooms to generate images for
const propertyRooms = () => {
    const rooms = [
        `exterior`,
        `bathroom`,
...
/**
 * Generates a random property configuration (num bedrooms, property type and is new build status) to use as a guide for the AI when generating property listings. 
 * This is used to ensure variety in the generated properties
 * @returns bedrooms, property type and new build status of the property listing to be generated by the AI
 */
...
async function uploadImagesToSuapbase(propertyId: number, buffer: Buffer, room: string) {
    ...
    const { error } = await supabase.storage
        ...
        console.log(`Uploaded image for property ${propertyId}, room: ${room}`);
    if (error) {
        throw error;
    }
}
